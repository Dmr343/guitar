import { CHROMATIC } from './theory.js';

export const OPEN_NOTES    = ['E','A','D','G','B','E'];
export const STRING_LABELS = ['E','A','D','G','B','e'];
export const FRET_MARKERS  = [3,5,7,9,12];
export const STRING_THICK  = [3.2,2.6,2.0,1.5,1.0,0.6];

export const FB_W      = 920;
export const FB_H      = 220;
export const FB_NUT    = 48;
export const FB_RIGHT  = FB_W - 16;
export const FB_STR_TOP = 28;
export const FB_STR_BOT = FB_H - 28;
export const FB_STR_GAP = (FB_STR_BOT - FB_STR_TOP) / 5;
...
export function fbInitBoard(svgEl, numFrets) {
...
export function chordToFbPositions(chord) {
...
export function fbRenderChordDots(svgEl, chord, color) {
...
// Draw a barre (cejilla) line at capoFret spanning fromSi to toSi
export function fbDrawBarre(svgEl, capoFret, fromSi, toSi, color) {
  const fretW = fbGetFretW(svgEl);
  const dotsG = fbGetDotsGroup(svgEl);
...
// Render a single dot colored by its functional role (R/3/5/7)
export function fbRenderFunctionalDot(svgEl, si, fret, funcName, noteName) {
  const fretW = fbGetFretW(svgEl);
  const dotsG = fbGetDotsGroup(svgEl);
